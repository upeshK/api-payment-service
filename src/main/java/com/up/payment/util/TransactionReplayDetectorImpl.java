...
public class TransactionReplayDetectorImpl implements TransactionReplayDetector {
	...
	/**
	 * Time-Based cleanup
	 * remove transaction tracking records which are older than cutoff from tracking queue as well as 
	 * @param cutoff
	 */
	private void cleanup(Instant cutoff) {		
		...
		while(true) {
			...
			recentTransactions.computeIfPresent(head.key, (key, timestamp) -> timestamp.equals(head.timestamp())? null : timestamp);
	...
	/**
	 * Build composite semantic key
	 * This defines when two transactions are semantically equivalent.
	 * @param txn
